class LoaderManager extends Laya.EventDispatcher {
	retryNum: number = 2;
	retryDelay: number = 0;
	maxLoader: number = 0;
	private _loaders: Array<any>;
	private _loaderCount: number = 0;
	private _resInfos: Array<any>;
	private _infoPool: Array<any>;
	...
	*/
	public create(url: string | Array<any>, complete: Laya.Handler, progress: Laya.Handler = null, clas: any = null, params: any = null, priority: number = 1, cache: boolean = true) {
		...
		if ((url instanceof Array)) {
			...
			for (var i = 0; i < itemCount; i++) {
				...
				item.progress = 0;
				var progressHandler = progress ? Laya.Handler.create(null, (item, value) => {
					item.progress = value;
					var num = 0;
				...
				var completeHandler = (progress || complete) ? Laya.Handler.create(null, (item, content) => {
					loadedCount++;
					item.progress = 1;
	...
	protected _create(url: string, complete: Laya.Handler, progress: Laya.Handler, clas: any, params: any, priority: number, cache: boolean) {
		(priority === void 0) && (priority = 1);
		(cache === void 0) && (cache = true);
		...
		if (!item) {
			...
			if (extension == "atlas") {
				...
				if (clas === Laya.Texture) type = "htmlimage";
				item = clas ? new clas() : null;
				if (item.hasOwnProperty("_loaded"))
					item._loaded = false;
				this.load(url, Laya.Handler.create(null, (data) => {
	...
	*/
	load(url: string | Array<any>, complete: Laya.Handler, progress: Laya.Handler = null, type: string = null, priority: number = 1, cache: boolean = true, group?, ignoreCache?) {
		(priority === void 0) && (priority = 1);
		(cache === void 0) && (cache = true);
	...
	protected _doLoad(resInfo: ResInfo) {
		this._loaderCount++;
	...
	protected onLoaded(resInfo: ResInfo, loader: Laya.Loader, data) {
		loader.offAll();
		loader._data = null;
	...
	protected onLoadError(resInfo: ResInfo, loader: Laya.Loader, msg) {
		this.onLoaded(resInfo, loader, null);
	}

	protected _endLoad(resInfo, content) {
		var url = resInfo.url;
		if (content == null) {
	...
	protected _addReTry = function (resInfo: ResInfo) {
	...
	*@param arr 简单：["a.png","b.png"]，复杂[{url:"a.png",type:Loader.IMAGE,size:100,priority:1},{url:"b.json",type:Loader.JSON,size:50,priority:1}]*/
	private _loadAssets(arr, complete, progress, type, priority, cache, group) {
		(priority === void 0) && (priority = 1);
		(cache === void 0) && (cache = true);
		...
		for (var i = 0; i < items.length; i++) {
			...
			var completeHandler = (complete || progress) ? Laya.Handler.create(null, (item, content) => {
				loadedCount++;
				item.progress = 1;
...
class ResInfo extends Laya.EventDispatcher {
	url: string = null;
	type: string = null;
	cache: boolean = false;
	group: string = null;
